...
def init_ol(P):
    ...
    fig = plt.figure()
    ax = plt.axes(xlim = (0, rho_j+10), ylim = (0, C))
...
def init_vsl(P):
    ...
    ax.plot(rho_1, q_out_1, 'b-', rho_2, q_out_2, 'b-', rho, q_in, 'y-', linewidth=2)
    line1, = ax.plot([], [], 'ro', ms = 10)
...
def animate_vsl(frame, line1, line2, line3, line4, P):
    rho = frame[0]
    q = frame[1:3]
    v = frame[3]
    d, C_d, C, epsilon, vf, w, wbar, rho_j, rho_jbar = P
...
def animation_vsl(P, rho_0, T, delta, Res):
    ...
    line1, line2, line3, line4, fig = init_vsl(P)
    if d < (1-epsilon)*C_d:
        omega = 1
    ...
    anim = animation.FuncAnimation(fig, animate_vsl, frames = frames, fargs = (line1, line2, line3, line4, P), interval = 20, blit = True, repeat=True)
